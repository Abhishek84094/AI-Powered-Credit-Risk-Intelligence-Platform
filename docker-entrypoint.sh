...
SEED_DB="/app/sql/credit_risk_seed.db"
DATA_DIR="/app/data"

# Check if SQLite analytical database exists and is non-empty
if [ ! -f "$DB_FILE" ] || [ ! -s "$DB_FILE" ]; then
    echo "[*] SQLite database ($DB_FILE) not found or empty."
    if [ -f "$SEED_DB" ] && [ -s "$SEED_DB" ]; then
        echo "[+] Found bundled analytical seed database ($SEED_DB). Activating..."
        cp "$SEED_DB" "$DB_FILE"
    elif [ -f "$DATA_DIR/application_train.csv" ] || [ -f "$DATA_DIR/home-credit-default-risk/application_train.csv" ]; then
